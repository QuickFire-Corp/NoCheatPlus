 ...
 * @author asofold
 */
class HotFixFallingBlockPortalEnter : Listener {
    /** Temporary use only: setWorld(null) after use.  */
    private val useLoc = Location(null, 0, 0, 0)
    private val wrapBlockCache // TODO: Fetch a getter from the registry.
            : WrapBlockCache

    init {
        testAvailability()
        wrapBlockCache = WrapBlockCache()
    }
    ...
    fun onEntityPortalEnter(event: EntityPortalEnterEvent) {
        ...
        val mat: Material?
        mat = if (entity is FallingBlock) {
            entity.material
        } else if (entity is Item) {
            // TODO: Not sure if needed.
            if (entity.itemStack.type.hasGravity()) {
                entity.itemStack.type
            } else {
                null
            }
        } else {
        ...
        if (mat != null) {
            val loc = entity.getLocation(useLoc)
            val world = loc!!.world
            val worldData = NCPAPIProvider.getNoCheatPlusAPI().worldDataManager.getWorldData(world)
            if (worldData.getGenericInstance(InventoryConfig::class.java).hotFixFallingBlockEndPortalActive) {
                val blockCache = wrapBlockCache.blockCache
                blockCache.setAccess(world)
                val nearbyPortal = BlockProperties.collidesId(
                    blockCache,
                    loc.x - 2.0, loc.y - 2.0, loc.z - 2.0,
                    loc.x + 3.0, loc.y + 3.0, loc.z + 3.0,
                ...
                if (nearbyPortal) {
                    ...
                    val message =
                        "[INVENTORY_HOTFIX] Remove falling block entering a portal near an end portal: " + mat + " at " + world!!.name + "/" + LocUtil.simpleFormatPosition(
                            loc
                        )
                    NCPAPIProvider.getNoCheatPlusAPI().logManager.info(Streams.STATUS, message)
                }
            }
            useLoc.world = null
        }
    ...
    companion object {
        ...
        fun testAvailability() {
            if (getClass("org.bukkit.event.entity.EntityPortalEnterEvent") == null
                || getClass("org.bukkit.entity.FallingBlock") == null
            ) {
