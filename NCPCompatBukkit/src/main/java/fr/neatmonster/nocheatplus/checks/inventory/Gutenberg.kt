...
class Gutenberg : Check(CheckType.INVENTORY_GUTENBERG), Listener {
    ...
    fun onEditBook(event: PlayerEditBookEvent) {
        ...
        val pages = newMeta.pageCount
        if (pages <= cc.gutenbergPageLimit) {
            // Legitimate.
            return
        }
        // Violation.
        val vl = pages - cc.gutenbergPageLimit
        data.gutenbergVL += vl.toDouble()
        if (executeActions(player, data.gutenbergVL, vl.toDouble(), cc.gutenbergActions).willCancel()) {
    ...
    companion object {
        ...
        fun testAvailability() {
            if (PlayerEditBookEvent::class.java.simpleName != "PlayerEditBookEvent") {
