 ...
 * @author asofold
 */
open class ReflectEntity @JvmOverloads constructor(
    base: ReflectBase?,
    reflectAxisAlignedBB: ReflectAxisAlignedBB?,
    damageSource: ReflectDamageSource?,
    obcClass: Class<*>? = Class.forName(
        base!!.obcPackageName + ".entity.CraftEntity"
    ),
    nmsClass: Class<*>? = Class.forName(base!!.nmsPackageName + ".Entity")
) : ReflectGetHandleBase<Entity?>(base, obcClass, nmsClass) {
    val nmsWidth: Field?
    val nmsLength // Something like height.
            : Field?

    @MostlyHarmless
    val nmsHeight // Not anymore in 1.11.
            : Field?

    // TODO: Pre 1.7.10 would be a field: nmsBoundingBox
    var nmsGetBoundingBox: Method? = null
    val nmsDead: Field?
    val nmsDamageEntity: Method?
    private val nmsDamageEntityNew: Method?

    @MostlyHarmless
    val nmsclearActiveItem: Method?
    var nmsDamageEntityInt = false

    init {
        // base

        // width, length (height)
        nmsWidth = getField(nmsClass, "width", Float::class.javaPrimitiveType)
        nmsLength = getField(nmsClass, "length", Float::class.javaPrimitiveType)
        nmsHeight = getField(nmsClass, "height", Float::class.javaPrimitiveType) // Rather old CB around 1.6.

        // dead
        nmsDead = getField(nmsClass, "dead", Boolean::class.javaPrimitiveType)

        // damageEntity(...)
        nmsDamageEntityNew = getMethod(nmsClass, "a", *arrayOf(damageSource!!.nmsClass, Float::class.javaPrimitiveType))
        nmsDamageEntity = nmsDamageEntityNew
            ?: getMethod(
                nmsClass, "damageEntity", arrayOf(
                    damageSource.nmsClass, Float::class.javaPrimitiveType
                ), arrayOf(damageSource.nmsClass, Int::class.javaPrimitiveType)
            )
        nmsDamageEntityInt = if (nmsDamageEntity != null) {
            nmsDamageEntity.parameterTypes[1] == Int::class.javaPrimitiveType
        } else {
            true // Uncertain.
        }
        nmsclearActiveItem = if (getMethodNoArgs(nmsClass, "clearActiveItem") == null) getMethodNoArgs(
            nmsClass,
            "eR"
        ) else getMethodNoArgs(nmsClass, "clearActiveItem")

        // getBoundingBox
        if (reflectAxisAlignedBB == null) {
            nmsGetBoundingBox = null
        } else {
            nmsGetBoundingBox = getMethodNoArgs(nmsClass, "getBoundingBox", reflectAxisAlignedBB.nmsClass)
        }
    }
