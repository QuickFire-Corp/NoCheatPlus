...
class BukkitFence(private val minXZ: Double, private val maxXZ: Double, private val height: Double) : BukkitShapeModel {
    ...
    override fun getShape(
        ...
        if (blockData is MultipleFacing) {
            ...
            val res = doubleArrayOf(minXZ, 0.0, minXZ, maxXZ, height, maxXZ)
            for (face in blockData.faces) {
                when (face) {
                    BlockFace.EAST -> res[3] = 1.0
                    BlockFace.NORTH -> res[2] = 0.0
                    BlockFace.WEST -> res[0] = 0.0
                    BlockFace.SOUTH -> res[5] = 1.0
                    else -> {}
                }
            }
            return res
