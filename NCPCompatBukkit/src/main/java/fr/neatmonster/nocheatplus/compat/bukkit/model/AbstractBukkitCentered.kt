 ...
 * @author asofold
 */
abstract class AbstractBukkitCentered(
    private val minDist: Double, length: Double,
    invertFace: Boolean
) : BukkitShapeModel {
    private val maxDist: Double
    private val length: Double
    protected val invertFace: Boolean
    ...
    protected abstract fun getFacing(blockData: BlockData?): BlockFace
    override fun getShape(
        ...
        val facing = if (invertFace) getFacing(blockData).oppositeFace else getFacing(blockData)
        when (facing) {
            ...
            BlockFace.NORTH -> return doubleArrayOf(
                minDist, minDist, 1.0 - length,
                maxDist, maxDist, 1.0
            )
            BlockFace.DOWN -> return doubleArrayOf(
                minDist, 1.0 - length, minDist,
                maxDist, 1.0, maxDist
            )
            BlockFace.UP -> return doubleArrayOf(
                minDist, 0.0, minDist,
