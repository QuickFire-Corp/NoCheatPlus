...
class BukkitChorusPlant : BukkitShapeModel {
    override fun getShape(
        ...
        if (Bridge1_13.hasBoundingBox()) {
            ...
            return doubleArrayOf(bd.minX - x, 0.1875, bd.minZ - z, bd.maxX - x, bd.maxY - y, bd.maxZ - z)
        }
        val blockData = block.blockData
        val res = doubleArrayOf(0.187, 0.188, 0.187, 1.0 - 0.187, 0.8125, 1.0 - 0.187)
        if (blockData is MultipleFacing) {
            for (face in blockData.faces) {
                when (face) {
                    BlockFace.UP -> res[4] = 1.0
