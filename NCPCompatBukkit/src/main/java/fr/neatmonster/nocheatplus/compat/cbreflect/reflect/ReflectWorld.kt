...
class ReflectWorld(
    base: ReflectBase, reflectMaterial: ReflectMaterial,
    reflectBlockPosition: ReflectBlockPosition?
) {
    /** World (nms)  */
    val nmsClass: Class<*>?
    val obcGetHandle: Method?
    var nmsGetType: Method? = null

    // nms - WorldServer: Used as IBlockAccess as well.
    init {
        val obcClass = Class.forName(base.obcPackageName + ".CraftWorld")
        obcGetHandle = getMethodNoArgs(obcClass, "getHandle")

        // IBlockData getType(BlockPosition): fail-safe.
        var reflectIBlockData: ReflectIBlockData? = null
        var nmsClass: Class<*>? = null
        try {
            reflectIBlockData = ReflectIBlockData(base, reflectMaterial)
        ...
        if (reflectIBlockData == null || nmsClass == null) {
            ...
            val method = getMethod(nmsClass, "getType", reflectBlockPosition!!.nmsClass)
            if (method!!.returnType == reflectIBlockData.nmsClass) {
                nmsGetType = method
            } else {
                nmsGetType = null
            }
     ...
     */
    fun nms_getType(nmsWorld: Any?, blockPosition: Any?): Any {
        if (nmsGetType == null) {
            throw ReflectFailureException()
        }
        return try {
            nmsGetType.invoke(nmsWorld, blockPosition)
