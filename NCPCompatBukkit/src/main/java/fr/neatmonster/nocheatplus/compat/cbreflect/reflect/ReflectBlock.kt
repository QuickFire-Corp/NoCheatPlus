...
class ReflectBlock(
    ...
    /** Block class.  */
    val nmsClass: Class<*>
    private val reflectAxisAlignedBB: ReflectAxisAlignedBB
    private val reflectIBlockData: ReflectIBlockData
    private val reflectIBlockAccess: ReflectIBlockAccess
    private val reflectWorld: ReflectWorld

    // Static.
    val nmsGetById: Method?

    // Instance.
    val nmsGetBlockData: Method?

    /** Not the original name.  */
    val nmsFetchAABB: Method?
    ...
    init {
        ...
        reflectAxisAlignedBB = ReflectAxisAlignedBB(base) // Fails if not available.
        reflectIBlockData = ReflectIBlockData(base, reflectMaterial) // Fails if not available.
        this.reflectWorld = reflectWorld
        if (reflectWorld.nmsClass == null || reflectWorld.nmsGetType == null) {
            fail()
        }
        reflectIBlockAccess = ReflectIBlockAccess(base)
        // Block.
        nmsClass = Class.forName(base!!.nmsPackageName + ".Block")
        // byID (static)
        nmsGetById = getMethod(nmsClass, "getById", Int::class.javaPrimitiveType)
        if (nmsGetById == null) {
            fail()
        }
        // Instance.
        nmsGetBlockData = getMethodNoArgs(nmsClass, "getBlockData", reflectIBlockData.nmsClass)
        if (nmsGetBlockData == null) {
            fail()
        }
        nmsFetchAABB = determine_fetchAABB()
     ...
     * @return
     */
    private fun determine_fetchAABB(): Method? {
        var nmsFetchAABB: Method? = null

        // At least since 1.11.
        if (ServerVersion.compareMinecraftVersion("1.11") >= 0) {
            nmsFetchAABB = getMethod(
                nmsClass, "b",
                reflectIBlockData.nmsClass, reflectIBlockAccess.nmsClass, reflectBlockPosition!!.nmsClass
            )
            if (nmsFetchAABB != null && nmsFetchAABB.returnType == reflectAxisAlignedBB.nmsClass && nmsFetchAABB.isAnnotationPresent(
     ...
     * @return
     */
    private fun nms_getBlockData(block: Any): Any {
        return try {
            nmsGetBlockData!!.invoke(block)
    ...
    override fun nms_fetchBounds(
        ...
        val pos = nmsBlockPosition(x, y, z)
        val blockData = reflectWorld.nms_getType(nmsWorld, pos)
        val bb = nms_fetchAABB(nmsBlock, blockData, nmsWorld, pos)
            ?: return doubleArrayOf(0.0, 0.0, 0.0, 1.0, 1.0, 1.0) // Special case.
        //return null;
        return reflectAxisAlignedBB.fillInValues(bb, DoubleArray(6))
    }

    private fun nms_fetchAABB(nmsBlock: Any, iBlockData: Any?, iBlockAccess: Any?, blockPosition: Any?): Any {
        return try {
            nmsFetchAABB!!.invoke(nmsBlock, iBlockData, iBlockAccess, blockPosition)
        } catch (e: IllegalAccessException) {
            throw ReflectFailureException()
        } catch (e: IllegalArgumentException) {
            throw ReflectFailureException()
    ...
    override val isFetchBoundsAvailable: Boolean
        get() =// Available, if initialized at all.
            true
}