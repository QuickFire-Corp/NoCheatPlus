...
class ReflectDamageSource(base: ReflectBase?) {
    var nmsClass: Class<*>? = null
    var nmsFALL: Any? = null

    init {
        try {
            val nmsClass = Class.forName(base!!.nmsPackageName + ".DamageSource")
            this.nmsClass = nmsClass
            val field = getField(nmsClass, "FALL", nmsClass)
            nmsFALL = if (field == null) null else ReflectionUtil[field, nmsClass, null]
        } catch (e: ClassNotFoundException) {
            val nmsClass = Class.forName("net.minecraft.world.damagesource.DamageSource")
            this.nmsClass = nmsClass
            val field = getField(nmsClass, "k", nmsClass)
            nmsFALL = if (field == null) null else ReflectionUtil[field, nmsClass, null]
        }
    }
}