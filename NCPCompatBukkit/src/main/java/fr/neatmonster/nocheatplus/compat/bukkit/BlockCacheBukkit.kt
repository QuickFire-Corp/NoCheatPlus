...
open class BlockCacheBukkit(world: World?) : BlockCache() {
    protected var world: World? = null

    /** Temporary use. Use LocUtil.clone before passing on. Call setWorld(null) after use.  */
    protected val useLoc = Location(null, 0, 0, 0)

    init {
        setAccess(world!!)
    }

    override fun setAccess(world: World): BlockCache {
        this.world = world
        if (world != null) {
            maxBlockY = world.maxHeight - 1
            minBlockY = BlockProperties.getMinWorldY()
        }
        return this
    }

    override fun fetchTypeId(x: Int, y: Int, z: Int): Material {
        // TODO: consider setting type id and data at once.
        return world!!.getBlockAt(x, y, z).type
    }

    override fun fetchData(x: Int, y: Int, z: Int): Int {
        // TODO: consider setting type id and data at once.
        return if (Bridge1_13.hasIsSwimming()) 0 else world!!.getBlockAt(x, y, z).data.toInt()
    }
    ...
    override fun standsOnEntity(
        ...
        try {
            ...
            for (other in entity.getNearbyEntities(2.0, 2.0, 2.0)) {
                val type = other.type
                if (type != EntityType.BOAT && type != EntityType.SHULKER) { //  && !(other instanceof Minecart)) 
                    continue
                }
                val locY = entity.getLocation(useLoc)!!.y
                useLoc.world = null
                return if (Math.abs(locY - minY) < 0.7) {
