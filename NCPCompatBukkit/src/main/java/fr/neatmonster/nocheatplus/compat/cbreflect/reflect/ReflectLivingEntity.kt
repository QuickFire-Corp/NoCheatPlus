...
open class ReflectLivingEntity @JvmOverloads constructor(
    base: ReflectBase?,
    reflectAxisAlignedBB: ReflectAxisAlignedBB?,
    damageSource: ReflectDamageSource?,
    obcClass: Class<*>? = Class.forName(
        base!!.obcPackageName + ".entity.CraftLivingEntity"
    ),
    nmsClass: Class<*>? = Class.forName(base!!.nmsPackageName + ".EntityLiving")
) : ReflectEntity(base, reflectAxisAlignedBB, damageSource, obcClass, nmsClass) {
    val nmsGetHealth: Method?

    init {
        nmsGetHealth = getMethodNoArgs(nmsClass, "getHealth")
    }
}