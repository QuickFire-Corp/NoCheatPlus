...
class ReflectPlayer @JvmOverloads constructor(
    base: ReflectBase?,
    reflectAxisAlignedBB: ReflectAxisAlignedBB?,
    damageSource: ReflectDamageSource?,
    obcClass: Class<*>? = Class.forName(
        base!!.obcPackageName + ".entity.CraftPlayer"
    ),
    nmsClass: Class<*>? = Class.forName(base!!.nmsPackageName + ".EntityPlayer")
) : ReflectLivingEntity(base, reflectAxisAlignedBB, damageSource, obcClass, nmsClass) {
    // Not sure: Living/Human entity.
    val nmsDeathTicks: Field?
    val nmsInvulnerableTicks: Field?
    val nmsGetAttributeInstance // TODO: LivingEntity
            : Method?

    init {
        // TODO: invulnerable etc.
        // deathTicks
        nmsDeathTicks = getField(nmsClass, "deathTicks", Int::class.javaPrimitiveType)
        nmsInvulnerableTicks = getField(nmsClass, "invulnerableTicks", Int::class.javaPrimitiveType)
        val method: Method?
        method = try {
            val clazzIAttribute = Class.forName(base!!.nmsPackageName + ".IAttribute")
            getMethod(nmsClass, "getAttributeInstance", clazzIAttribute)
