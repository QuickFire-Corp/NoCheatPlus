...
class BukkitTrapDoor : BukkitShapeModel {
    override fun getShape(
        ...
        if (blockData is TrapDoor) {
            ...
            if (trapDoor.isOpen) {
                when (trapDoor.facing) {
                    BlockFace.NORTH -> return doubleArrayOf(0.0, 0.0, 1.0 - openWidth, 1.0, 1.0, 1.0)
                    BlockFace.SOUTH -> return doubleArrayOf(0.0, 0.0, 0.0, 1.0, 1.0, openWidth)
                    BlockFace.EAST -> return doubleArrayOf(0.0, 0.0, 0.0, openWidth, 1.0, 1.0)
                    BlockFace.WEST -> return doubleArrayOf(1.0 - openWidth, 0.0, 0.0, 1.0, 1.0, 1.0)
                    else -> {}
                }
            } else {
                return if (trapDoor.half == Half.BOTTOM) doubleArrayOf(
                    0.0,
                    0.0,
                    0.0,
                    1.0,
                    closedHeight,
                    1.0
                ) else doubleArrayOf(0.0, 1.0 - closedHeight, 0.0, 1.0, 1.0, 1.0)
            }
        }
    ...
    override fun getFakeData(
        ...
        if (blockData is TrapDoor) {
            ...
            if (trapDoor.isOpen) {
                when (trapDoor.facing) {
                    BlockFace.EAST -> return 3
                    BlockFace.NORTH -> return 4
                    BlockFace.SOUTH -> return 5
                    BlockFace.WEST -> return 6
                    else -> {}
                }
            }
        }
    ...
    companion object {
        private const val closedHeight = 0.1875
        private const val openWidth = 0.1875
    }
}