...
class BukkitLevelled : BukkitShapeModel {
    override fun getShape(blockCache: BlockCache?, world: World, x: Int, y: Int, z: Int): DoubleArray? {
        ...
        val blockDataAbove = blockAbove.blockData
        return if (BlockFlags.getBlockFlags(
                world.getBlockAt(
                    x,
                    y,
                    z
                ).type
            ) and BlockFlags.F_WATER != 0L && blockDataAbove is Waterlogged && blockDataAbove.isWaterlogged
        ) {
            doubleArrayOf(0.0, 0.0, 0.0, 1.0, 1.0, 1.0)
        } else doubleArrayOf(0.0, 0.0, 0.0, 1.0, (8 / 9f).toDouble(), 1.0)
    }
    ...
    override fun getFakeData(blockCache: BlockCache?, world: World, x: Int, y: Int, z: Int): Int {
        ...
        return if (blockData is Levelled) {
            blockData.level
        } else 0
    }
}