...
class BukkitGate(private val minXZ: Double, private val maxXZ: Double, private val height: Double) : BukkitShapeModel {
    constructor(inset: Double, height: Double) : this(inset, 1.0 - inset, height) {}
        ...
        if (blockData is Directional) {
            when (blockData.facing) {
                BlockFace.WEST -> return doubleArrayOf(
                    minXZ, 0.0, 0.0, maxXZ, height, 1.0
                )
                BlockFace.EAST -> return doubleArrayOf(minXZ, 0.0, 0.0, maxXZ, height, 1.0)
                BlockFace.NORTH -> return doubleArrayOf(0.0, 0.0, minXZ, 1.0, height, maxXZ)
                BlockFace.SOUTH -> return doubleArrayOf(0.0, 0.0, minXZ, 1.0, height, maxXZ)
                else -> {}
            }
        }
    ...
    override fun getFakeData(
        ...
        val state = block.state
        val blockData = state.blockData
        return if (blockData is Openable) {
            if (blockData.isOpen) 0x4 else 0
        } else {
            0
        }
    }
