...
class BukkitDirectionalCentered(
    inset: Double, length: Double,
    invertFace: Boolean
) : AbstractBukkitCentered(inset, length, invertFace) {
    override fun getFacing(blockData: BlockData?): BlockFace {
        return if (blockData is Directional) {
            blockData.facing
        } else {
            BlockFace.SELF
        }
