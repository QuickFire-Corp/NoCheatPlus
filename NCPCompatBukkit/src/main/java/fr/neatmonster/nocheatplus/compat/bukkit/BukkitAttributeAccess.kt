...
class BukkitAttributeAccess : IAttributeAccess {
    init {
        if (getClass("org.bukkit.attribute.AttributeInstance") == null) {
            throw RuntimeException("Service not available.")
        }
    }

    private fun operationToInt(operation: AttributeModifier.Operation): Int {
        return when (operation) {
            AttributeModifier.Operation.ADD_NUMBER -> 0
            AttributeModifier.Operation.ADD_SCALAR -> 1
            AttributeModifier.Operation.MULTIPLY_SCALAR_1 -> 2
     ...
     * @return
     */
    private fun getModifier(attrInst: AttributeInstance?, id: UUID): AttributeModifier? {
        for (mod in attrInst!!.modifiers) {
            if (id == mod.uniqueId) {
                return mod
            }
        }
    ...
    private fun getMultiplier(mod: AttributeModifier): Double {
        return AttribUtil.getMultiplier(operationToInt(mod.operation), mod.amount)
    }

    override fun getSpeedAttributeMultiplier(player: Player): Double {
        val attrInst = player.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED)
        val `val` = attrInst!!.value / attrInst.baseValue
        val mod = getModifier(attrInst, AttribUtil.ID_SPRINT_BOOST)
        return if (mod == null) `val` else `val` / getMultiplier(mod)
    }

    override fun getSprintAttributeMultiplier(player: Player): Double {
        val attrInst = player.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED)
        val mod = getModifier(attrInst, AttribUtil.ID_SPRINT_BOOST)
        return mod?.let { getMultiplier(it) } ?: 1.0
    }
}