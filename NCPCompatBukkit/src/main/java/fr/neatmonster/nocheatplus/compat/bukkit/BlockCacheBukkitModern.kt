 ...
 * @author asofold
 */
class BlockCacheBukkitModern : BlockCacheBukkit {
    private var shapeModels: Map<Material, BukkitShapeModel>? = null

    constructor(shapeModels: Map<Material, BukkitShapeModel>?) : super(null) {
        this.shapeModels = shapeModels
    }

    constructor(world: World?) : super(world) {}

    override fun fetchData(x: Int, y: Int, z: Int): Int {
        val mat = getType(x, y, z)
        val shapeModel = shapeModels!![mat]
        if (shapeModel != null) {
            val data = shapeModel.getFakeData(this, world, x, y, z)
    ...
    override fun fetchBounds(x: Int, y: Int, z: Int): DoubleArray {
        ...
        val mat = getType(x, y, z)
        val shapeModel = shapeModels!![mat]
        return if (shapeModel == null) {
            super.fetchBounds(x, y, z)
        } else {
            shapeModel.getShape(this, world, x, y, z)!!
        }
    }

    override fun standsOnEntity(
        entity: Entity,
        minX: Double,
        minY: Double,
        minZ: Double,
        ...
        try {
            ...
            for (vehicle in entity.getNearbyEntities(0.1, 2.0, 0.1)) {
                val type = vehicle.type
                if (!MaterialUtil.isBoat(type) && type != EntityType.SHULKER) { //  && !(vehicle instanceof Minecart)) 
                    continue
                }
                val vehicleY = vehicle.getLocation(useLoc)!!.y + vehicle.height
                val entityY = entity.getLocation(useLoc)!!.y
                useLoc.world = null
                return if (vehicleY < entityY + 0.1 && Math.abs(vehicleY - entityY) < 0.7) {
