...
class BukkitSlab : BukkitShapeModel {
    override fun getShape(
        ...
        if (blockData is Slab) {
            when (blockData.type) {
                Slab.Type.BOTTOM -> return doubleArrayOf(0.0, 0.0, 0.0, 1.0, 0.5, 1.0)
                Slab.Type.TOP -> return doubleArrayOf(0.0, 0.5, 0.0, 1.0, 1.0, 1.0)
                Slab.Type.DOUBLE -> {}
                else -> {}
            }
        }
