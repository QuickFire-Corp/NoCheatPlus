 ...
 * @author asofold
 */
class ReflectEntityLastPositionAndLook @JvmOverloads constructor(
    base: ReflectBase = ReflectBase(),
    obcClass: Class<*>? = Class.forName(base.obcPackageName + ".entity.CraftEntity"),
    nmsClass: Class<*>? = Class.forName(base.nmsPackageName + ".Entity")
) : ReflectGetHandleBase<Entity?>(base, obcClass, nmsClass), IEntityAccessLastPositionAndLook {
    val lastX: Field?
    val lastY: Field?
    val lastZ: Field?
    val lastYaw: Field?
    ...
    init {
        lastX = getField(nmsClass, "lastX", Double::class.javaPrimitiveType)
        lastY = getField(nmsClass, "lastY", Double::class.javaPrimitiveType)
        lastZ = getField(nmsClass, "lastZ", Double::class.javaPrimitiveType)
        lastYaw = getField(nmsClass, "lastYaw", Float::class.javaPrimitiveType)
        lastPitch = getField(nmsClass, "lastPitch", Float::class.javaPrimitiveType)
        validateNotNull(lastX, lastY, lastZ, lastYaw, lastPitch)
    }

    override fun getPositionAndLook(entity: Entity, location: ISetPositionWithLook) {
        try {
            performGet(entity, location)
        } catch (t: Throwable) {
            NCPAPIProvider.getNoCheatPlusAPI().logManager.warning(
                Streams.STATUS,
                "Could not retrieve last position and look for Entity: " + entity.javaClass.name
            )
        }
    }

    @Throws(IllegalArgumentException::class, IllegalAccessException::class)
    private fun performGet(entity: Entity, location: ISetPositionWithLook) {
        val nmsObject = getHandle(entity)
    ...
    override fun setPositionAndLook(entity: Entity, location: IGetPositionWithLook) {
        try {
            performSet(entity, location)
        } catch (t: Throwable) {
            NCPAPIProvider.getNoCheatPlusAPI().logManager.warning(
                Streams.STATUS,
                "Could not set last position and look for Entity: " + entity.javaClass.name
            )
        }
    }

    @Throws(IllegalArgumentException::class, IllegalAccessException::class)
    private fun performSet(entity: Entity, location: IGetPositionWithLook) {
        val nmsObject = getHandle(entity)
        lastX!!.setDouble(nmsObject, location.x)
        lastY!!.setDouble(nmsObject, location.y)
        lastZ!!.setDouble(nmsObject, location.z)
        lastYaw!!.setFloat(nmsObject, location.yaw)
        lastPitch!!.setFloat(nmsObject, location.pitch)
    }

    override fun fail() {
        throw RuntimeException("Not available.") // To be caught internally.
    }
}