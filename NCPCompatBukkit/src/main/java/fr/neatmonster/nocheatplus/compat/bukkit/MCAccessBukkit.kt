...
open class MCAccessBukkit : MCAccessBukkitBase(), BlockPropertiesSetup {
    override fun setupBlockProperties(worldConfigProvider: WorldConfigProvider<*>?) {
        ...
        if (!itchyBlocks.isEmpty()) {
            StaticLog.logDebug("The following blocks can not be modeled correctly: " + join(itchyBlocks, ", "))
        }
    }
}