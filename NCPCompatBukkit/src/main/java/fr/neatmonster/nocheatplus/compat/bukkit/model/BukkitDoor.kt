...
class BukkitDoor : BukkitShapeModel {
    override fun getShape(
        ...
        if (blockData is Door) {
            ...
            val doorWidth = doorWidthOpen // isOpen ? doorWidthOpen : doorWidthClosed;
            when (getWhereTheDoorIsFace(
                door.facing,
                door.hinge, isOpen
            )) {
                BlockFace.NORTH -> return doubleArrayOf(0.0, 0.0, 0.0, 1.0, 1.0, doorWidth)
                BlockFace.SOUTH -> return doubleArrayOf(0.0, 0.0, 1.0 - doorWidth, 1.0, 1.0, 1.0)
                BlockFace.EAST -> return doubleArrayOf(1.0 - doorWidth, 0.0, 0.0, 1.0, 1.0, 1.0)
                BlockFace.WEST -> return doubleArrayOf(0.0, 0.0, 0.0, doorWidth, 1.0, 1.0)
                else -> {}
            }
        }
    ...
    companion object {
         ...
         * where the door is.
         */
        private fun getWhereTheDoorIsFace(
            facing: BlockFace, hinge: Hinge,
            isOpen: Boolean
        ): BlockFace {
            // For idiots:
            /*
        ...
        private const val doorWidthOpen = 0.1875
    }
}