...
class BukkitPiston : BukkitShapeModel {
    override fun getShape(
        ...
        val res = doubleArrayOf(0.0, 0.0, 0.0, 1.0, 1.0, 1.0)
        if (blockData is Piston) {
            val piston = blockData
            if (!piston.isExtended) return res
            val face = piston.facing
            when (face) {
                BlockFace.NORTH -> res[2] = 0.25
                BlockFace.SOUTH -> res[5] = 0.75
                BlockFace.WEST -> res[0] = 0.25
                BlockFace.EAST -> res[3] = 0.75
                BlockFace.UP -> res[4] = 0.75
                BlockFace.DOWN -> res[1] = 0.25
                else -> {}
            }
            return res
        }
