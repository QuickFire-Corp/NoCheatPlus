...
class BlockCacheCBReflect(// TODO: Not sure if reflection can gain speed over Bukkit API anywhere (who wants to try?).
    protected val helper: ReflectHelper, world: World?
) : BlockCacheBukkit(world) {
    protected var nmsWorld: Any? = null
    override fun setAccess(world: World): BlockCache {
        super.setAccess(world)
        nmsWorld = if (world == null) null else helper.getHandle(world)
        return this
    }

    override fun fetchBounds(x: Int, y: Int, z: Int): DoubleArray {
        val mat = getType(x, y, z)
        val shape = LegacyBlocks.getShape(this, mat, x, y, z, true)
        return shape
            ?: try {
                helper.nmsWorld_fetchBlockShape(nmsWorld, this.getType(x, y, z), x, y, z)!!
