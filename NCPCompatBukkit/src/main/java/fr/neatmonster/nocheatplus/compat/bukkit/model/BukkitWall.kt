...
class BukkitWall(
    private val minXZ: Double,
    private val maxXZ: Double,
    private val height: Double,
    private val sideInset: Double
) : BukkitShapeModel {
    private val east: DoubleArray
    private val north: DoubleArray
    private val west: DoubleArray
    private val south: DoubleArray
    private val eastwest: DoubleArray
    private val southnorth: DoubleArray

    constructor(inset: Double, height: Double) : this(inset, 1.0 - inset, height, 0.0) {}
    constructor(inset: Double, height: Double, sideInset: Double) : this(inset, 1.0 - inset, height, sideInset) {}

    init {
        east = doubleArrayOf(maxXZ, 0.0, sideInset, 1.0, height, 1.0 - sideInset)
        north = doubleArrayOf(sideInset, 0.0, 0.0, 1.0 - sideInset, height, minXZ)
        west = doubleArrayOf(0.0, 0.0, sideInset, minXZ, height, 1.0 - sideInset)
        south = doubleArrayOf(sideInset, 0.0, maxXZ, 1.0 - sideInset, height, 1.0)
        eastwest = doubleArrayOf(0.0, 0.0, sideInset, 1.0, height, 1.0 - sideInset)
        southnorth = doubleArrayOf(sideInset, 0.0, 0.0, 1.0 - sideInset, height, 1.0)
    }
    ...
    override fun getShape(
        ...
        if (blockData is MultipleFacing) {
            val fence = blockData
            var res = doubleArrayOf(minXZ, 0.0, minXZ, maxXZ, height, maxXZ)
            val a = fence.faces
            if (!a.contains(BlockFace.UP) && a.size == 2) {
                if (a.contains(BlockFace.SOUTH)) {
                    return doubleArrayOf(sideInset, 0.0, 0.0, 1.0 - sideInset, height, 1.0)
                }
                if (a.contains(BlockFace.WEST)) {
                    return doubleArrayOf(0.0, 0.0, sideInset, 1.0, height, 1.0 - sideInset)
                }
            }
            for (face in fence.faces) {
                when (face) {
                    BlockFace.EAST -> res = add(res, east)
                    BlockFace.NORTH -> res = add(res, north)
                    BlockFace.WEST -> res = add(res, west)
                    BlockFace.SOUTH -> res = add(res, south)
                    else -> {}
                }
            }
            return res
        } else if (blockData is Wall) {
            val wall = blockData
            if (wall.isUp) {
                var res = doubleArrayOf(minXZ, 0.0, minXZ, maxXZ, height, maxXZ)
                if (wall.getHeight(BlockFace.WEST) != Wall.Height.NONE) res = add(res, west)
                if (wall.getHeight(BlockFace.EAST) != Wall.Height.NONE) res = add(res, east)
                if (wall.getHeight(BlockFace.NORTH) != Wall.Height.NONE) res = add(res, north)
                if (wall.getHeight(BlockFace.SOUTH) != Wall.Height.NONE) res = add(res, south)
                return res
            } else {
                var res: DoubleArray? = null
                if (wall.getHeight(BlockFace.WEST) != Wall.Height.NONE && wall.getHeight(BlockFace.EAST) != Wall.Height.NONE) {
                    if (res == null) res = eastwest
                }
                if (wall.getHeight(BlockFace.NORTH) != Wall.Height.NONE && wall.getHeight(BlockFace.SOUTH) != Wall.Height.NONE) {
                    res = res?.let { add(it, southnorth) } ?: southnorth
                }
