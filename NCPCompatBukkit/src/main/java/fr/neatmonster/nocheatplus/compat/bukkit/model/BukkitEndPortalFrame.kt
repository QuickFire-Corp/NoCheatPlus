...
class BukkitEndPortalFrame : BukkitShapeModel {
    override fun getShape(
        ...
        return if (blockData is EndPortalFrame) {
            getShape(blockData.hasEye())
        } else {
            doubleArrayOf(0.0, 0.0, 0.0, 1.0, 1.0, 1.0)
        }
    }

    private fun getShape(hasEye: Boolean): DoubleArray {
        return if (hasEye) doubleArrayOf(
            0.0, 0.0, 0.0, 1.0, 0.8125, 1.0,
            0.25, 0.8125, 0.25, 0.75, 1.0, 0.75
        ) else doubleArrayOf(0.0, 0.0, 0.0, 1.0, 0.8125, 1.0)
    }
