...
class Fight(plugin: Plugin?) : BaseAdapter(plugin, ListenerPriority.MONITOR, *initPacketTypes()) {
    //@Override
    ...
    private fun handleVelocityPacket(event: PacketEvent) {
        if (event.isPlayerTemporary) return
        if (event.packetType !== PacketType.Play.Server.EXPLOSION) return
        ...
        val velX = floats.read(1)
        val velY = floats.read(2)
        val velZ = floats.read(3)
        if (Math.abs(velX).toDouble() == 0.0 && Math.abs(velZ).toDouble() == 0.0 && Math.abs(velY)
                .toDouble() == 0.0
        ) return
        ...
        data.explosionVelAxisX += velX.toDouble()
        data.explosionVelAxisY += velY.toDouble()
        data.explosionVelAxisZ += velZ.toDouble()
    ...
    companion object {
        private fun initPacketTypes(): Array<PacketType> {
            val types: List<PacketType> = LinkedList(
                Arrays.asList( //PacketType.Play.Client.ARM_ANIMATION,
