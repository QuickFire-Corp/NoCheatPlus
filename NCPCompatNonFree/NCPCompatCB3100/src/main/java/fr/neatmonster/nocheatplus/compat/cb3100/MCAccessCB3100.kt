...
class MCAccessCB3100 : MCAccess {
    /**
     * Constructor to let it fail.
     */
    init {
        commandMap
        ReflectionUtil.checkMembers("net.minecraft.server.v1_7_R4.", arrayOf("Entity", "dead"))
        // block bounds, original: minX, maxX, minY, maxY, minZ, maxZ
        ReflectionUtil.checkMethodReturnTypesNoArgs(
            net.minecraft.server.v1_7_R4.Block::class.java,
            arrayOf("x", "y", "z", "A", "B", "C"),
    ...
    fun resetActiveItem(player: Player): Boolean {
        (player as CraftPlayer).getHandle().bA()
        return true
    }
}