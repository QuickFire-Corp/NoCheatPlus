...
class MCAccessSpigotCB1_8_R1 : MCAccess {
    /**
     * Constructor to let it fail.
     */
    init {
        commandMap
        ReflectionUtil.checkMembers("net.minecraft.server.v1_8_R1.", arrayOf("Entity", "dead"))
        // block bounds, original: minX, maxX, minY, maxY, minZ, maxZ
        ReflectionUtil.checkMethodReturnTypesNoArgs(
            net.minecraft.server.v1_8_R1.Block::class.java,
            arrayOf("z", "A", "B", "C", "D", "E"),
    ...
    fun getBlockCache(world: World?): BlockCache {
        return BlockCacheSpigotCB1_8_R1(world)
    }
    ...
    fun resetActiveItem(player: Player): Boolean {
        (player as CraftPlayer).getHandle().bT()
        return true
    }
}