...
class AttributeAccess : IAttributeAccess {
    ...
    fun getSpeedAttributeMultiplier(player: Player): Double {
        val attr: AttributeInstance = (player as CraftLivingEntity).getHandle().a(GenericAttributes.d)
        val `val`: Double = attr.e() / attr.b()
    ...
    fun getSprintAttributeMultiplier(player: Player): Double {
        val mod: AttributeModifier =
            (player as CraftLivingEntity).getHandle().a(GenericAttributes.d).a(AttribUtil.ID_SPRINT_BOOST)
        return if (mod == null) {
            1.0
        } else {
