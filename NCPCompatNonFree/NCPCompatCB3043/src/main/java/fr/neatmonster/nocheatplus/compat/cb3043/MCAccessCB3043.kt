...
class MCAccessCB3043 : MCAccess {
    /**
     * Constructor to let it fail.
     */
    init {
        commandMap
        ReflectionUtil.checkMembers("net.minecraft.server.v1_7_R3.", arrayOf("Entity", "dead"))
        // block bounds, original: minX, maxX, minY, maxY, minZ, maxZ
        ReflectionUtil.checkMethodReturnTypesNoArgs(
            net.minecraft.server.v1_7_R3.Block::class.java,
            arrayOf("x", "y", "z", "A", "B", "C"),
    ...
    fun resetActiveItem(player: Player): Boolean {
        (player as CraftPlayer).getHandle().bz()
        return true
    }
}