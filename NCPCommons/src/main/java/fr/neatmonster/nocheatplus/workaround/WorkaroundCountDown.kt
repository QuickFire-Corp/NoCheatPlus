...
class WorkaroundCountDown(
    ...
    init {
        ...
        stageCounter = AcceptDenyCounter()
    }
    ...
    override fun resetConditions() {
        currentCount = initialCount
        stageCounter.resetCounter()
    }

    override fun getNewInstance(): WorkaroundCountDown? {
        return setParentCounters(WorkaroundCountDown(id, initialCount))
    }

    override fun getStageCounter(): IAcceptDenyCounter? {
        return stageCounter
    }

    override fun testUse(isUse: Boolean): Boolean {
        return if (!isUse) {
            currentCount > 0
        } else {
            if (currentCount > 0) {
