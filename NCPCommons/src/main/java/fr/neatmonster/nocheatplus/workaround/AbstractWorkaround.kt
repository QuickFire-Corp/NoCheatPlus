 ...
 * @author asofold
 */
abstract class AbstractWorkaround(private val id: String?) : IWorkaround {
    private val allTimeCounter: AcceptDenyCounter?
     ...
     */
    init {
        allTimeCounter = AcceptDenyCounter()
    }
    ...
    override fun getAllTimeCounter(): IAcceptDenyCounter? {
     ...
     * @return
     */
    abstract fun testUse(isUse: Boolean): Boolean
     ...
     */
    fun <W : IWorkaround?> setParentCounters(instance: W?): W? {
        val allTimeParent = allTimeCounter.getParentCounter()
        if (allTimeParent != null) {
            val instanceAllTime = instance.getAllTimeCounter()
            if (instance is ICounterWithParent) {
                (instanceAllTime as ICounterWithParent).parentCounter = allTimeParent
            }
        }
        return instance
    }
