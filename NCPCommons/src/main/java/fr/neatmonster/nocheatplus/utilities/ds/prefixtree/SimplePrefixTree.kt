...
class SimplePrefixTree<K> : PrefixTree<K?, PrefixTree.SimpleNode<K?>?, SimpleLookupEntry<K?>?>(
    NodeFactory { SimpleNode() },
    LookupEntryFactory { node, insertion, depth, hasPrefix -> SimpleLookupEntry(node, insertion, depth, hasPrefix) }) {
    class SimpleLookupEntry<K>(node: SimpleNode<K?>?, insertion: SimpleNode<K?>?, depth: Int, hasPrefix: Boolean) :
        LookupEntry<K?, SimpleNode<K?>?>(node, insertion, depth, hasPrefix)
}