...
class QueueRORAWithLock<E> @JvmOverloads constructor(private val lock: Lock? = ReentrantLock()) : IQueueRORA<E?> {
    ...
    override fun removeAll(): MutableList<E?>? {
        lock.lock()
        val result: MutableList<E?>? = elements
        elements = LinkedList()
        lock.unlock()
        return result
    }

    override fun reduce(maxSize: Int): Int {
        var dropped = 0
        lock.lock()
        val size = elements.size
        if (size <= maxSize) {
