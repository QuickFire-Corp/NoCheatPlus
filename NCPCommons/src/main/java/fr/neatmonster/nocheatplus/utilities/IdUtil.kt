...
object IdUtil {
    /**
     * Valid user name check (Minecraft).<br></br>
     ...
     */
    @kotlin.jvm.JvmStatic
    fun UUIDFromStringSafe(input: String?): UUID? {
        if (input == null) {
            return null
        }
        try {
    ...
    /**
     * More flexible UUID parsing.<br></br>
     * (Taken from TrustCore.)
     *
     * @param input
     * the input
    ...
    fun UUIDFromString(input: String?): UUID? {
        ...
        return if (len == 36) {
            ...
            UUID.fromString(String(newChars))
        } else {
            throw IllegalArgumentException("Unexpected length ($len) for uuid: $input")
        }
    }
}