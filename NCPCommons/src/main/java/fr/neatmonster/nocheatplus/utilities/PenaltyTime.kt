...
class PenaltyTime {
     ...
     */
    fun applyPenalty(now: Long, duration: Long) {
        penaltyLast = now
        penaltyEnd = if (now < penaltyLast) {
            now + duration
        } else {
            Math.max(now + duration, penaltyEnd)
        }
    }
     ...
     * @return
     */
    fun isPenalty(now: Long): Boolean {
        return if (now < penaltyLast) {
            resetPenalty()
            false
        } else {
            now < penaltyEnd
