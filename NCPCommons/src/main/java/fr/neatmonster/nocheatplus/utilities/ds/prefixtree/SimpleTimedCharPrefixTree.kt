...
class SimpleTimedCharPrefixTree(access: Boolean) :
    TimedCharPrefixTree<SimpleTimedCharNode?, SimpleTimedCharLookupEntry?>(
        NodeFactory { parent ->
            val ts: Long
            ts = parent?.ts ?: System.currentTimeMillis()
            SimpleTimedCharNode(ts)
        },
        LookupEntryFactory { node, insertion, depth, hasPrefix ->
            SimpleTimedCharLookupEntry(
                node,
                insertion,
                depth,
                hasPrefix
    ...
    class SimpleTimedCharLookupEntry(
        node: SimpleTimedCharNode?,
        insertion: SimpleTimedCharNode?,
        depth: Int,
        hasPrefix: Boolean
    ) : TimedCharLookupEntry<SimpleTimedCharNode?>(node, insertion, depth, hasPrefix)
}