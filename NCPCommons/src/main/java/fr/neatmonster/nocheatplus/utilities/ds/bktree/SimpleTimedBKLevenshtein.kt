...
class SimpleTimedBKLevenshtein : TimedBKLevenshtein<SimpleTimedLevenNode?, STBKLResult?>(
    NodeFactory { value, parent -> SimpleTimedLevenNode(value) },
    LookupEntryFactory { nodes, match, distance, isNew -> STBKLResult(nodes, match, distance, isNew) }
) {
    class STBKLResult(
        nodes: MutableCollection<SimpleTimedLevenNode?>?,
        match: SimpleTimedLevenNode?,
        distance: Int,
        isNew: Boolean
    ) : LookupEntry<CharArray?, SimpleTimedLevenNode?>(nodes, match, distance, isNew)
}