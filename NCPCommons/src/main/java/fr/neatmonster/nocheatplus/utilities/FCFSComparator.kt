 ...
 * @author dev1mc
 */
class FCFSComparator<T> @JvmOverloads constructor(
    comparators: MutableCollection<Comparator<T?>?>?,
    reverse: Boolean = false
) : Comparator<T?> {
    private val comparators: MutableList<Comparator<T?>?>?
    private val reverse: Boolean

    init {
        this.comparators = ArrayList(comparators)
        this.reverse = reverse
    }

    override fun compare(o1: T?, o2: T?): Int {
        for (i in comparators.indices) {
            val res = comparators.get(i).compare(o1, o2)
            if (res != 0) {
