...
open class TimedCharPrefixTree<N : TimedCharNode<N?>?, L : TimedCharLookupEntry<N?>?>(
    nodeFactory: NodeFactory<Char?, N?>?,
    resultFactory: LookupEntryFactory<Char?, N?, L?>?,
    access: Boolean
) : CharPrefixTree<N?, L?>(nodeFactory, resultFactory) {
    open class TimedCharLookupEntry<N : TimedCharNode<N?>?>(node: N?, insertion: N?, depth: Int, hasPrefix: Boolean) :
        CharLookupEntry<N?>(node, insertion, depth, hasPrefix) {
        @kotlin.jvm.JvmField
        var timeInsertion: LongArray? = null
    }

    open class TimedCharNode<N : TimedCharNode<N?>?>(ts: Long) : CharNode<N?>() {
        var ts: Long = 0
    ...
    class SimpleTimedCharNode(ts: Long) : TimedCharNode<SimpleTimedCharNode?>(ts)

    protected var ts: Long = 0
    protected var timeInsertion: LongArray? = LongArray(200)
    protected val access: Boolean
    protected var updateTime = false
    protected var depth = 0
    protected var arrayGrowth = 1.3f
    ...
    override fun lookup(keys: MutableList<Char?>?, create: Boolean): L? {
    ...
    override fun visit(node: N?) {
        if (depth == timeInsertion.size) {
            // This might be excluded by contract.
            timeInsertion = Arrays.copyOf(timeInsertion, (timeInsertion.size * arrayGrowth).toInt())
        }
        timeInsertion.get(depth) = node.ts
        if (updateTime) node.ts = ts
        depth++
    }

    override fun decorate(result: L?) {
    ...
    companion object {
         ...
         * @return
         */
        fun newTimedCharPrefixTree(access: Boolean): TimedCharPrefixTree<SimpleTimedCharNode?, TimedCharLookupEntry<SimpleTimedCharNode?>?>? {
            return TimedCharPrefixTree(
                { parent ->
                    val ts: Long
                    ts = parent?.ts ?: System.currentTimeMillis()
                    SimpleTimedCharNode(ts)
                },
                { node, insertion, depth, hasPrefix -> TimedCharLookupEntry(node, insertion, depth, hasPrefix) },
                access
            )
        }
    }
