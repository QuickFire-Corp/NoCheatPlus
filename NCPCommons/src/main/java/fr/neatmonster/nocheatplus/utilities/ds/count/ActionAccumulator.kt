 ...
 * @author asofold
 */
class ActionAccumulator(nBuckets: Int, bucketCapacity: Int) {
    /** Event counter.  */
    ...
    fun add(value: Float) {
        ...
        if (counts.get(0) >= bucketCapacity) {
            shift()
        }
     ...
     * @return
     */
    fun bucketCount(bucket: Int): Int {
     ...
     * @return
     */
    fun toInformalString(): String? {
        val b = StringBuilder(buckets.size * 10)
        b.append("|")
        for (i in buckets.indices) {
            b.append(StringUtil.fdec3.format(buckets.get(i).toDouble()) + "/" + counts.get(i) + "|")
        }
        return b.toString()
    }
}