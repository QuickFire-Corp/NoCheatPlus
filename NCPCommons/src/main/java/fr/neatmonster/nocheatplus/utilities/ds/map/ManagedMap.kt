...
open class ManagedMap<K, V>(defaultCapacity: Int, loadFactor: Float) {
    protected inner class ValueWrap(value: V?) {
        var ts: Long
        @kotlin.jvm.JvmField
        var value: V?
    ...
    @kotlin.jvm.JvmField
    protected val map: LinkedHashMap<K?, ValueWrap?>?

    init {
        map = LinkedHashMap(defaultCapacity, loadFactor, true)
    }
    ...
    fun put(key: K?, value: V?): V? {
        val wrap = map.get(key)
        return if (wrap == null) {
            map[key] = ValueWrap(value)
            null
        } else {
            val res = wrap.value
            wrap.value = value
     ...
     * @return
     */
    open fun expire(ts: Long): MutableCollection<K?>? {
        val rem: MutableList<K?> = LinkedList()
        for ((key, wrap) in map) {
            if (wrap.ts < ts) rem.add(key) else break
        }
        for (key in rem) {
            map.remove(key)
        }
