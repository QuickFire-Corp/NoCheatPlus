...
/**
 * Mapping Class<T> to  for arbitrary T, aiming at simple generic
 * caches.
 * <hr></hr>
 * Until as specialized implementation is present, see [HashMapLOW] for
 * reference.
 *
 * @author asofold
</T> */
class InstanceMapLOW(lock: Lock?, initialSize: Int) {
    /*
     * TODO: Optimized implementation possible (performance/JIT)? Buckets and
     * entries could support the type relation.
     */
    private val map: HashMapLOW<Class<*>?, Any?>?

    init {
        map = HashMapLOW(lock, 12)
    }

    fun <T, I : T?> put(key: Class<T?>?, value: I?): T? {
        return map.put(key, value) as T?
    }

    fun <T, I : T?> putIfAbsent(key: Class<T?>?, value: I?): T? {
        return map.putIfAbsent(key, value) as T?
    }
    ...
    fun <T> getLocked(key: Class<T?>?): T? {
        return map.getLocked(key) as T?
    }
    ...
    fun containsKeyLocked(key: Class<*>?): Boolean {
        return map.containsKeyLocked(key)
    }

    fun getKeys(): MutableCollection<Class<*>?>? {
        return map.getKeys()
    }
     ...
     * @param keys
     */
    fun remove(keys: MutableCollection<Class<*>?>?) {
        map.remove(keys)
    }
