...
class LogOptions     // TODO: shutdown policy (clear, log), rather with per-world threads.
    (val name: String?, val callContext: CallContext?) {
    /**
     * Describe the context in which the log method may be used.<br></br>
     * Policies:
    ...
    constructor(options: LogOptions?) : this(options.name, options.callContext) {}
}