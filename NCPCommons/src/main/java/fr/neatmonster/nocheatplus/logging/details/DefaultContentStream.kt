...
class DefaultContentStream<C>(private val dispatcher: LogNodeDispatcher?) : ContentStream<C?> {
    private var nodes: ArrayList<LogNode<C?>?>? = ArrayList()
    override fun log(level: Level?, content: C?) {
        val nodes = nodes
    ...
    override fun addNode(node: LogNode<C?>?) {
    ...
    override fun clear() {
        synchronized(nodes) { nodes.clear() }
    }
}