...
class FileLoggerAdapter @JvmOverloads constructor(file: File?, prefix: String? = null) : FileLogger(file),
     ...
     */
    init {
        this.prefix = if (prefix == null || prefix.isEmpty()) null else prefix
    }

    override fun log(level: Level?, content: String?) {
        // TODO: Check loggerisInoperable() ?
        logger.log(level, if (prefix == null) content else prefix + content)
    }
}