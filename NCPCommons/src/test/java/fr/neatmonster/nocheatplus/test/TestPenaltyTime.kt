...
class TestPenaltyTime {
    @Test
    fun testZeroSequence() {
        val now = System.currentTimeMillis()
        val pt = PenaltyTime()
        pt.applyPenalty(now, 0)
        if (pt.isPenalty(now)) {
            Assert.fail("Expect no penalty with duration 0.")
        }
    }
        ...
        for (i in 0..9999) {
            ...
            if (j == 0L) {
                if (pt.isPenalty(now + i)) {
                    Assert.fail("Expect no penalty at i=$i")
                }
                pt.applyPenalty(now + i, 50)
            } else if (j < 50) {
                if (!pt.isPenalty(now + i)) {
                    Assert.fail("Expect penalty at i=$i")
                }
            } else {
                if (pt.isPenalty(now + i)) {
                    Assert.fail("Expect no penalty at i=$i")
    ...
    fun testReset() {
        ...
        if (pt.isPenalty(now - 1)) {
            Assert.fail("isPenalty should return false on past time.")
        }
        pt.applyPenalty(now - 1, 73)
        if (pt.isPenalty(now + 72)) {
            Assert.fail("isPenalty should not return edge time after reset.")
        }
    }
}