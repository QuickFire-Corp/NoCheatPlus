...
class TestActionFrequency {
    @Test
    fun testSum() {
        val freq = ActionFrequency(10, 100)
        for (i in 0..9) {
            freq.setBucket(i, 1f)
        }
        if (freq.score(1f) != 10f) Assert.fail("10x1=10")
        freq.clear(0)
        if (freq.score(1f) != 0f) Assert.fail("clear=0")

        // TODO: more tests...
    }

    @Test
    fun testAddFromZeroTime() {
        addFromTime(0)
    }

    @Test
    fun testAddFromCurrentTime() {
     ...
     */
    fun addFromTime(time: Long) {
        val freq = ActionFrequency(3, 333)
        freq.update(time)
        for (i in 0..998) {
            freq.add(time + i, 1f)
            // TODO: maybe test sums here already.
        }
        if (freq.score(1f) != 999f) Assert.fail("Sum should be 999, got instead: " + freq.score(1f))
        freq.update(time + 999)
        if (freq.score(1f) != 666f) Assert.fail("Sum should be 666, got instead: " + freq.score(1f))
        freq.update(time + 1332)
        if (freq.score(1f) != 333f) Assert.fail("Sum should be 333, got instead: " + freq.score(1f))
        freq.update(time + 1665)
        if (freq.score(1f) != 0f) Assert.fail("Sum should be 0, got instead: " + freq.score(1f))
    }

    @Test
    fun testUpdateAlternatingSignumTimes() {
        // Basically fails if this generates an exception.
        var sig = 1
        val freq = ActionFrequency(10, 100)
        for (i in 0..999) {
            freq.update((i * sig).toLong())
            sig = sig * -1
        }
    }
}