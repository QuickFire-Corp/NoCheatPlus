...
class TestSimpleCharPrefixTree {
    private val feed = Arrays.asList(
        "op", "op dummy", "ncp info"
    )
    private val mustFind = Arrays.asList(
        "op", "op dummy", "ncp info", "ncp info test"
    )
    private val mustNotFindWords = Arrays.asList(
        "opp", "opp dummy", "op dummy2", "ncp", "ncp dummy"
    )
    private val mustNotFindPrefix = Arrays.asList(
        "ok", "ncp", "ncp dummy", "ncp inf"
    )

    /** Numbers are neither prefix nor suffix of each other.  */
    private val uniqueNumbers = Arrays.asList(
        "123456", "2345678", "34567", "456789"
    )

    @Test
    fun testPrefixWords() {
        val tree = SimpleCharPrefixTree()
        tree.feedAll(feed, false, true)
        for (input in mustFind) {
            if (!tree.hasPrefixWords(input)) {
                Assert.fail("Expect to be matched: '$input'")
            }
        }
        for (input in mustNotFindWords) {
            if (tree.hasPrefixWords(input)) {
                Assert.fail("Expect not to be matched: '$input'")
            }
        }
    }
    ...
    fun testHasPrefix() {
        ...
        for (input in mustNotFindPrefix) {
            if (tree.hasPrefix(input)) {
                Assert.fail("Expect not to be matched: '$input'")
            }
        }
        ...
        if (!tree.hasPrefix("ncp infocrabs")) {
            Assert.fail("'ncp info' should be a prefix of 'ncp infocrabs'.")
        }
    }

    @Test
    fun testSuffixTree() {
        val prefixTree = SimpleCharPrefixTree()
        prefixTree.feedAll(uniqueNumbers, false, true)
        val suffixTree = SimpleCharPrefixTree()
        for (key in uniqueNumbers) {
            suffixTree.feed(StringUtil.reverse(key))
        }
        for (input in uniqueNumbers) {
            if (!prefixTree.hasPrefix(input)) {
                Assert.fail("Fed data not matching prefix tree: $input")
            }
            if (suffixTree.hasPrefix(input)) {
                Assert.fail("Non-reversed data is matching suffix tree: $input")
            }
        }
        for (input in uniqueNumbers) {
            input = StringUtil.reverse(input)
            if (prefixTree.hasPrefix(input)) {
                Assert.fail("Reversed fed data is matching prefix tree: $input")
            }
            if (!suffixTree.hasPrefix(input)) {
                Assert.fail("Reversed fed data not matching suffix tree: $input")
            }
        }
    }
}